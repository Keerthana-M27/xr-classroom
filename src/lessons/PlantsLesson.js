...
function PlantsLesson({ name }) {
  const navigate = useNavigate()
  const [slide, setSlide] = useState(0)
  const [mascotText, setMascotText] = useState('Hi! I am Leafy! Tap me! 🌿')
  ...
  const slides = [
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '30px', color: '#FFD700' }}>Plants make their own food using sunlight!</h2>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '20px' }}>What do plants need? 🌿</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            {[['☀️','Sunlight'],['💧','Water'],['🪨','Soil'],['💨','Air']].map(([e,t]) => (
              <div key={t} style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '15px', padding: '20px', textAlign: 'center' }}>
                <div style={{ fontSize: '40px' }}>{e}</div>
                <div style={{ fontSize: '18px', marginTop: '5px', color: '#FFD700' }}>{t}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>Parts of a Plant! 🌸</h2>
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>🌸 Flower — makes seeds!</div>
            <div>🍃 Leaves — make food from sunlight!</div>
            <div>🌿 Stem — carries water up!</div>
            <div>🌱 Roots — drink water from soil!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>Amazing Facts! 🌟</h2>
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>🍃 Leaves are green because of Chlorophyll!</div>
            <div>🌳 Trees give us oxygen to breathe!</div>
            <div>🌻 Sunflowers always face the Sun!</div>
            <div>🌵 Cactus stores water in its stem!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '35px', color: '#FFD700' }}>You know all about Plants!</h2>
  ...
  const mascotTexts = [
    'Hi! I am Leafy! Tap me! 🌿',
    'Plants need sunlight and water! ☀️',
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 20; i++) {
      const geo = i % 3 === 0
        ...
        : new THREE.SphereGeometry(Math.random() * 0.7 + 0.3, 32, 32)
      const mat = new THREE.MeshStandardMaterial({ color: colors[Math.floor(Math.random() * colors.length)], roughness: 0.6 })
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom, #0f0c29, #302b63, #24243e)', color: 'white', fontFamily: 'Arial, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <h2 style={{ color: '#FFD700', fontSize: '22px' }}>🌱 Plants — Slide {slide + 1} of {slides.length}</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '30px' }}>
          ...
          {slide < slides.length - 1
            ...
            : <button onClick={() => navigate('/games/plants', { state: { name } })} style={{ backgroundColor: '#4ECDC4', color: 'white', fontSize: '20px', fontWeight: 'bold', padding: '15px 40px', borderRadius: '50px', border: 'none', cursor: 'pointer' }}>🎮 Play Game!</button>}
        </div>
      </div>
