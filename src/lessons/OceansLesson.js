...
function OceansLesson({ name }) {
  const navigate = useNavigate()
  const [slide, setSlide] = useState(0)
  const [mascotText, setMascotText] = useState('Hi! I am Wavey! Tap me! 🌊')
  ...
  const slides = [
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '30px', color: '#FFD700' }}>Oceans are huge bodies of salt water!</h2>
          <p style={{ fontSize: '20px', color: '#d8b4fe', marginTop: '10px' }}>They cover 71% of our Earth! 💧</p>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>The 5 Oceans! 🗺️</h2>
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>🏆 <span style={{ color: '#FFD700' }}>Pacific</span> — BIGGEST ocean!</div>
            <div>🌊 <span style={{ color: '#4ECDC4' }}>Atlantic</span> — 2nd biggest!</div>
            <div>🇮🇳 <span style={{ color: '#96CEB4' }}>Indian</span> — near India!</div>
            <div>🥶 <span style={{ color: '#45B7D1' }}>Arctic</span> — near North Pole!</div>
            <div>❄️ <span style={{ color: '#DDA0DD' }}>Southern</span> — near Antarctica!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '20px' }}>Ocean Animals! 🐋</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            {[['🐋','Blue Whale — largest!'],['🦈','Shark — fastest fish!'],['🐬','Dolphin — smartest!'],['🐙','Octopus — 8 arms!']].map(([e,t]) => (
              <div key={t} style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '15px', padding: '15px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '15px', marginTop: '5px', color: '#FFD700' }}>{t}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>💧 Oceans cover 71% of Earth!</div>
            <div>🏆 Pacific is bigger than all land!</div>
            <div>🧂 Ocean water is salty!</div>
            <div>🐋 Blue Whale is the biggest animal ever!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '35px', color: '#FFD700' }}>You know the Oceans!</h2>
  ...
  const mascotTexts = [
    'Hi! I am Wavey! Tap me! 🌊',
    'Pacific is the biggest ocean! 🏆',
    'Dolphins are the smartest! 🐬',
    'Oceans cover 71% of Earth! 💧',
    'You are an Ocean Explorer! 🏆',
  ]
  ...
  useEffect(() => {
    ...
    const objects = []
    const colors = [0x0984E3, 0x74B9FF, 0x00CEC9, 0x0652DD, 0x1289A7]

    for (let i = 0; i < 20; i++) {
      const geo = new THREE.TorusGeometry(Math.random()*0.8+0.3, 0.2, 16, 100)
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom, #0f0c29, #302b63, #24243e)', color: 'white', fontFamily: 'Arial, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <h2 style={{ color: '#FFD700', fontSize: '22px' }}>🌊 Oceans — Slide {slide+1} of {slides.length}</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '30px' }}>
          ...
          {slide < slides.length-1
            ...
            : <button onClick={() => navigate('/games/oceans', { state: { name } })} style={{ backgroundColor: '#4ECDC4', color: 'white', fontSize: '20px', fontWeight: 'bold', padding: '15px 40px', borderRadius: '50px', border: 'none', cursor: 'pointer' }}>🎮 Play Game!</button>}
        </div>
      </div>
