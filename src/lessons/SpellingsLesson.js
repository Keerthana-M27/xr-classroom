...
function SpellingsLesson({ name }) {
  const navigate = useNavigate()
  const [slide, setSlide] = useState(0)
  const [mascotText, setMascotText] = useState('Hi! I am Spelly! Tap me! ✏️')
  ...
  const slides = [
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '30px', color: '#FFD700' }}>Spellings help us write words correctly!</h2>
          <p style={{ fontSize: '20px', color: '#d8b4fe', marginTop: '10px' }}>Every word has its own spelling! 📖</p>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '20px' }}>3 Letter Words! 🌟</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            {[['🐱','C - A - T = CAT'],['🐶','D - O - G = DOG'],['☀️','S - U - N = SUN'],['🏠','H - U - T = HUT']].map(([e,t]) => (
              <div key={t} style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '15px', padding: '15px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '16px', color: '#FFD700', marginTop: '8px', fontWeight: 'bold' }}>{t}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '20px' }}>More Words! 📖</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            {[['🐝','B - E - E = BEE'],['🌸','R - O - S - E = ROSE'],['🍎','A - P - P - L - E'],['📚','B - O - O - K = BOOK']].map(([e,t]) => (
              <div key={t} style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '15px', padding: '15px', textAlign: 'center' }}>
                <div style={{ fontSize: '40px' }}>{e}</div>
                <div style={{ fontSize: '15px', color: '#FFD700', marginTop: '8px', fontWeight: 'bold' }}>{t}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>Tips to Remember! 💡</h2>
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>🔊 Say the word out loud!</div>
            <div>✍️ Write it 3 times!</div>
            <div>📚 Read books every day!</div>
            <div>🎮 Play spelling games!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '35px', color: '#FFD700' }}>You are a Spelling Champion!</h2>
  ...
  const mascotTexts = [
    'Hi! I am Spelly! Tap me! ✏️',
    'Sound out each letter! 🔊',
    'Read books to learn spellings! 📚',
    'Write it 3 times to remember! ✍️',
  ...
  useEffect(() => {
    ...
    const objects = []
    const colors = [0x4ECDC4, 0xFFD700, 0xFF6B6B, 0x96CEB4, 0x45B7D1]

    for (let i = 0; i < 20; i++) {
      const geo = new THREE.BoxGeometry(Math.random()*1+0.5, Math.random()*1.5+0.5, 0.2)
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom, #0f0c29, #302b63, #24243e)', color: 'white', fontFamily: 'Arial, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <h2 style={{ color: '#FFD700', fontSize: '22px' }}>✏️ Spellings — Slide {slide+1} of {slides.length}</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '30px' }}>
          ...
          {slide < slides.length-1
            ...
            : <button onClick={() => navigate('/games/spellings', { state: { name } })} style={{ backgroundColor: '#4ECDC4', color: 'white', fontSize: '20px', fontWeight: 'bold', padding: '15px 40px', borderRadius: '50px', border: 'none', cursor: 'pointer' }}>🎮 Play Game!</button>}
        </div>
      </div>
...
export default SpellingsLesson