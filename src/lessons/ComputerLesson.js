...
function ComputerLesson({ name }) {
  const navigate = useNavigate()
  const [slide, setSlide] = useState(0)
  const [mascotText, setMascotText] = useState('Hi! I am Byte! Tap me! 💻')
  ...
  const slides = [
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '30px', color: '#FFD700' }}>A computer is an electronic machine that helps us!</h2>
          <p style={{ fontSize: '20px', color: '#d8b4fe', marginTop: '10px' }}>It can do many amazing things! 🌟</p>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '20px' }}>Parts of a Computer! 🖥️</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            {[['🖥️','Monitor — the screen!'],['⌨️','Keyboard — for typing!'],['🖱️','Mouse — for clicking!'],['🖨️','CPU — the brain!']].map(([e,t]) => (
    ...
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>What can computers do? 🌟</h2>
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>📝 Help us write and type!</div>
            <div>🎨 Help us draw pictures!</div>
            <div>🎮 Play fun games!</div>
            <div>📚 Help us learn new things!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>Computer Safety Rules! 📌</h2>
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>✅ Always ask an adult before using!</div>
            <div>✅ Do not share your name online!</div>
            <div>✅ Take breaks every 30 minutes!</div>
            <div>✅ Keep the computer clean!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '35px', color: '#FFD700' }}>You know about Computers!</h2>
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 20; i++) {
      const geo = new THREE.BoxGeometry(Math.random()*1.2+0.4, Math.random()*0.8+0.3, 0.15)
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom, #0f0c29, #302b63, #24243e)', color: 'white', fontFamily: 'Arial, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <h2 style={{ color: '#FFD700', fontSize: '22px' }}>💻 What is Computer — Slide {slide+1} of {slides.length}</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '30px' }}>
          ...
          {slide < slides.length-1
            ...
            : <button onClick={() => navigate('/games/computer', { state: { name } })} style={{ backgroundColor: '#4ECDC4', color: 'white', fontSize: '20px', fontWeight: 'bold', padding: '15px 40px', borderRadius: '50px', border: 'none', cursor: 'pointer' }}>🎮 Play Game!</button>}
        </div>
      </div>
