...
function FunGamesLesson({ name }) {
  const navigate = useNavigate()
  const [slide, setSlide] = useState(0)
  const [mascotText, setMascotText] = useState('Hi! I am Gamey! Tap me! 🎮')
  ...
  const slides = [
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '30px', color: '#FFD700' }}>Computer games help us learn and have fun!</h2>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '20px' }}>Types of Games! 🌟</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            {[['🧩','Puzzle — think hard!'],['🏃','Action — move fast!'],['📚','Learning — get smarter!'],['🎨','Creative — make things!']].map(([e,t]) => (
    ...
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>Gaming Rules! 📌</h2>
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>⏰ Take a break every 30 minutes!</div>
            <div>👨‍👩‍👧 Always ask a parent first!</div>
            <div>🚫 Never talk to strangers online!</div>
            <div>✅ Play educational games!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>Benefits of Games! 💡</h2>
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>🧠 Puzzle games make brain stronger!</div>
            <div>🤝 Team games teach teamwork!</div>
            <div>📖 Learning games teach subjects!</div>
            <div>🎨 Creative games build imagination!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '35px', color: '#FFD700' }}>You know about Fun Games!</h2>
  ...
  useEffect(() => {
    ...
    const objects = []
    const colors = [0x6C5CE7, 0xFF6B6B, 0x4ECDC4, 0xFFD700, 0x96CEB4]
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom, #0f0c29, #302b63, #24243e)', color: 'white', fontFamily: 'Arial, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <h2 style={{ color: '#FFD700', fontSize: '22px' }}>🎮 Fun Games — Slide {slide+1} of {slides.length}</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '30px' }}>
          ...
          {slide < slides.length-1
            ...
            : <button onClick={() => navigate('/games/fungames', { state: { name } })} style={{ backgroundColor: '#4ECDC4', color: 'white', fontSize: '20px', fontWeight: 'bold', padding: '15px 40px', borderRadius: '50px', border: 'none', cursor: 'pointer' }}>🎮 Play Game!</button>}
        </div>
      </div>
