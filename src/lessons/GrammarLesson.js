...
function GrammarLesson({ name }) {
  const navigate = useNavigate()
  const [slide, setSlide] = useState(0)
  const [mascotText, setMascotText] = useState('Hi! I am Grammo! Tap me! 📝')
  ...
  const slides = [
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '30px', color: '#FFD700' }}>Grammar helps us speak and write correctly!</h2>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '20px' }}>What is a NOUN? 🏷️</h2>
          <div style={{ backgroundColor: 'rgba(255,107,107,0.3)', borderRadius: '20px', padding: '25px' }}>
            <p style={{ fontSize: '20px', color: 'white', marginBottom: '15px' }}>A Noun is the NAME of a person, place or thing!</p>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '30px', fontSize: '22px', color: '#FFD700' }}>
              <div>👦 Boy</div>
              <div>🏫 School</div>
              <div>🍎 Apple</div>
            </div>
    ...
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>VERB and ADJECTIVE! 🌟</h2>
          <div style={{ backgroundColor: 'rgba(78,205,196,0.2)', borderRadius: '20px', padding: '20px', marginBottom: '15px' }}>
            <p style={{ fontSize: '18px', color: '#4ECDC4', marginBottom: '8px', fontWeight: 'bold' }}>VERB = Action word!</p>
            <div style={{ fontSize: '22px', color: 'white' }}>🏃 Run &nbsp; 🍽️ Eat &nbsp; 💤 Sleep</div>
          </div>
          <div style={{ backgroundColor: 'rgba(255,215,0,0.2)', borderRadius: '20px', padding: '20px' }}>
            <p style={{ fontSize: '18px', color: '#FFD700', marginBottom: '8px', fontWeight: 'bold' }}>ADJECTIVE = Describing word!</p>
            <div style={{ fontSize: '22px', color: 'white' }}>🔴 Red &nbsp; 🐘 Big &nbsp; 😊 Happy</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '15px' }}>Sentence Rules! 📌</h2>
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>✅ Start with a Capital Letter!</div>
            <div>✅ End with a Full Stop (.)</div>
            <div>✅ Make complete sense!</div>
          </div>
          <div style={{ marginTop: '20px', backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '15px', padding: '15px' }}>
            <div style={{ fontSize: '20px', color: '#4ECDC4' }}>The cat sits on the mat.</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '35px', color: '#FFD700' }}>You know Grammar!</h2>
  ...
  const mascotTexts = [
    'Hi! I am Grammo! Tap me! 📝',
    'Nouns are names of things! 🏷️',
    'Verbs are action words! 🏃',
    'Always start with Capital letter! ✅',
    'You are a Grammar Star! 🏆',
  ]
  ...
  useEffect(() => {
    ...
    const objects = []
    const colors = [0x45B7D1, 0x4ECDC4, 0xFFD700, 0xFF6B6B, 0xDDA0DD]

    for (let i = 0; i < 18; i++) {
      const geo = i % 2 === 0
        ? new THREE.BoxGeometry(Math.random()*1.2+0.4, Math.random()*1.2+0.4, 0.15)
        : new THREE.SphereGeometry(Math.random()*0.6+0.3, 32, 32)
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom, #0f0c29, #302b63, #24243e)', color: 'white', fontFamily: 'Arial, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <h2 style={{ color: '#FFD700', fontSize: '22px' }}>📝 Grammar — Slide {slide+1} of {slides.length}</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '30px' }}>
          ...
          {slide < slides.length-1
            ...
            : <button onClick={() => navigate('/games/grammar', { state: { name } })} style={{ backgroundColor: '#4ECDC4', color: 'white', fontSize: '20px', fontWeight: 'bold', padding: '15px 40px', borderRadius: '50px', border: 'none', cursor: 'pointer' }}>🎮 Play Game!</button>}
        </div>
      </div>
