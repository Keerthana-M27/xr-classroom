...
function HumanBodyLesson({ name }) {
  const navigate = useNavigate()
  const [slide, setSlide] = useState(0)
  const [mascotText, setMascotText] = useState('Hi! I am Hearty! Tap me! ❤️')
  ...
  const slides = [
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '30px', color: '#FFD700' }}>Our body is an amazing machine!</h2>
          <p style={{ fontSize: '20px', color: '#d8b4fe', marginTop: '10px' }}>Many parts work together to keep us alive! 💪</p>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '20px' }}>Important Body Parts! 🌟</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            {[['🧠','Brain — controls everything!'],['❤️','Heart — pumps blood!'],['🫁','Lungs — help us breathe!'],['🦴','Bones — give us shape!']].map(([e,t]) => (
              <div key={t} style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '15px', padding: '15px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '15px', marginTop: '5px' }}>{t}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '26px', color: '#FFD700', marginBottom: '20px' }}>Our 5 Senses! 👁️</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
            {[['👁️','Eyes — See!'],['👂','Ears — Hear!'],['👃','Nose — Smell!'],['👅','Tongue — Taste!'],['🤚','Skin — Touch!']].map(([e,t]) => (
              <div key={t} style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '15px', padding: '15px', textAlign: 'center' }}>
      ...
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '20px', lineHeight: '2.2', color: 'white' }}>
            <div>❤️ Heart beats 100,000 times every day!</div>
            <div>🦴 Adults have 206 bones!</div>
            <div>🧠 Brain uses 20% of our energy!</div>
            <div>👁️ Eyes can see millions of colours!</div>
          </div>
        </div>
      )
    },
    {
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '35px', color: '#FFD700' }}>You know the Human Body!</h2>
  ...
  const mascotTexts = [
    'Hi! I am Hearty! Tap me! ❤️',
    'Your brain controls everything! 🧠',
    'You have 5 amazing senses! 👁️',
    'Your heart never stops beating! ❤️',
    'You are a Body Expert! 🏆',
  ]
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(to bottom, #0f0c29, #302b63, #24243e)', color: 'white', fontFamily: 'Arial, sans-serif', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <h2 style={{ color: '#FFD700', fontSize: '22px' }}>🫀 Human Body — Slide {slide + 1} of {slides.length}</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '30px' }}>
          ...
          {slide < slides.length - 1
            ...
            : <button onClick={() => navigate('/games/humanbody', { state: { name } })} style={{ backgroundColor: '#4ECDC4', color: 'white', fontSize: '20px', fontWeight: 'bold', padding: '15px 40px', borderRadius: '50px', border: 'none', cursor: 'pointer' }}>🎮 Play Game!</button>}
        </div>
      </div>
