...
function MultiplicationGame() {
  ...
  const [mascotText, setMascotText] = useState('Fill the multiplication table! ⭐')
  ...
  const generateQuestion = () => {
    ...
    const wrong3 = answer + Math.floor(Math.random() * 8) + 3
    const opts = [answer, wrong1, wrong2 > 0 ? wrong2 : wrong3 + 2, wrong3]
  ...
  const handleAnswer = (option) => {
    ...
    if (option === question.answer) {
      setScore(score + 10)
      setFeedback('correct')
      setMascotText('Brilliant! You are a Math genius! 🌟')
      setTimeout(() => {
      ...
      setLives(lives - 1)
      setFeedback('wrong')
      setMascotText('Keep trying! You can do it! 💪')
      if (lives - 1 <= 0) {
  ...
  const renderStars = (count) => {
    return Array(Math.min(count, 10)).fill('⭐').join(' ')
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <h2 style={{ color: '#FFD700', fontSize: '22px' }}>✖️ Multiplication Game</h2>
          ...
          {question && (
            <>
              ...
              <div style={{ marginBottom: '20px' }}>
                {Array(question.a).fill(0).map((_, i) => (
                  <div key={i} style={{ fontSize: '30px', lineHeight: '1.5' }}>
                    {Array(question.b).fill('⭐').join(' ')}
                  </div>
                ))}
              </div>
              ...
              <p style={{ color: '#d8b4fe', fontSize: '18px', marginBottom: '20px' }}>
                {question.a} rows of {question.b} stars = ?
              </p>
