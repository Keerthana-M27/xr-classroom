...
function SubtractionGame() {
  ...
  const [mascotText, setMascotText] = useState('Pop the right balloon! 🎈')
  const [feedback, setFeedback] = useState(null)
  const [gameOver, setGameOver] = useState(false)
  const [round, setRound] = useState(1)

  const balloonColors = ['#FF6B6B', '#4ECDC4', '#FFD700', '#96CEB4']
  ...
  const handleAnswer = (option) => {
    ...
    if (option === question.answer) {
      setScore(score + 10)
      setFeedback('correct')
      setMascotText('Great pop! You are a star! ⭐')
      setTimeout(() => {
      ...
      setLives(lives - 1)
      setFeedback('wrong')
      setMascotText('Oops wrong balloon! Try again! 💪')
      if (lives - 1 <= 0) {
  ...
  const renderBalloons = (count, color) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <h2 style={{ color: '#FFD700', fontSize: '22px' }}>➖ Subtraction Game</h2>
          ...
          {question && (
            <>
              ...
              <div style={{ fontSize: '40px', marginBottom: '10px', lineHeight: '1.8' }}>
                {renderBalloons(question.a)}
              </div>
              <div style={{ fontSize: '35px', color: '#FFD700', marginBottom: '10px' }}>
                ➖ {question.b} balloons popped! 💥
              </div>
